...
func main() {
	...
	r.HandleFunc("/api/command", commandHandler).Methods("POST")
...
func commandHandler(w http.ResponseWriter, r *http.Request) {
	...
	output, err := executeHauler(req.Command, req.Args...)
	if err != nil {
...
func fileUploadHandler(w http.ResponseWriter, r *http.Request) {
	...
	var destPath string
	if fileType == "haul" {
		destPath = filepath.Join("/data/hauls", handler.Filename)
	} else {
		destPath = filepath.Join("/data/manifests", handler.Filename)
	}
...
func fileDownloadHandler(w http.ResponseWriter, r *http.Request) {
	...
	if fileType == "haul" {
		...
		filePath = filepath.Join("/data/manifests", filename)
	}
...
func keyUploadHandler(w http.ResponseWriter, r *http.Request) {
	...
	keyPath := filepath.Join("/data/config/keys", handler.Filename)
	...
	io.Copy(dst, file)
	respondJSON(w, Response{Success: true, Output: "Key uploaded: " + handler.Filename})
}
...
func tlsCertUploadHandler(w http.ResponseWriter, r *http.Request) {
	...
	certPath := filepath.Join("/data/config/certs", handler.Filename)
	...
	io.Copy(dst, file)
	respondJSON(w, Response{Success: true, Output: "TLS cert uploaded: " + handler.Filename})
}
...
func valuesUploadHandler(w http.ResponseWriter, r *http.Request) {
	...
	valuesPath := filepath.Join("/data/config/values", handler.Filename)
	...
	io.Copy(dst, file)
	respondJSON(w, Response{Success: true, Output: "Values file uploaded: " + handler.Filename})
}
